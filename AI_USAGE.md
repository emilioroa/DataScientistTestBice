...
Documento vivo: se actualiza en cada etapa del desafío. Última actualización: **etapa 2 — modelado, calibración y política de aprobación**.
...
### 8. La IA sobrevendió el hallazgo del umbral por plazo

**Qué afirmó.** Al derivar el umbral óptimo, la IA presentó el resultado de que `p*` depende del plazo (9,8% a 12 meses, 30,4% a 48) como el gran hallazgo de la sección económica, con la frase "un corte único deja valor sobre la mesa". El álgebra es correcta —la verifiqué a mano, incluido el resultado no trivial de que el monto se cancela— pero la afirmación sobre su impacto no estaba medida, solo argumentada.

**Qué encontré al cuantificarlo.** Monté el backtest con la economía real (§9.2), con un diseño de tres bloques temporales para que el umbral global se eligiera *fuera* de la muestra de evaluación. El umbral por plazo rinde 1.057 MM contra 1.031 MM del mejor corte único: **+2,5%**. Es real y es defendible —la curva de §9.3 muestra que supera a cualquier corte global posible— pero es un refinamiento, no el titular.

**Qué hice.** Reordené la narrativa de la sección 9. El titular medido es otro: pasar de aprobar todo a aplicar **cualquier** umbral sensato lleva la ganancia de 337 a ~1.057 MM, **+214%**. Eso es lo que va primero en el informe para el gerente de Riesgo; el umbral por plazo queda como el párrafo fino que sigue. También agregué las tres limitaciones que la versión original omitía: que la ganancia sobre test es estimada porque no tiene target, el sesgo de selección (*reject inference*) por trabajar solo con créditos desembolsados, y que el óptimo asume capital ilimitado.

**Por qué importa.** El enunciado pide cuantificar el impacto de negocio, y una brecha entre lo que declaro y lo que el número aguanta es exactamente lo que se penaliza. Prefiero llevar un +214% medido y honesto que un hallazgo elegante cuyo peso real nunca comprobé.

---

### 9. La IA propuso descartar `canal` por derivar en el tiempo

**Qué propuso.** Tras la validación adversarial (§8.2), la IA presentó el resultado como "la deriva está concentrada en `canal` y la variable con fuga", sugiriendo tratar ambas del mismo modo y recortar el AUC adversarial quitando las dos.

**Por qué está mal.** El argumento no se sostiene: `canal` es una variable legítima, disponible al momento de la solicitud, y su cambio en el tiempo **es exactamente la información que explica el deterioro de la cartera** (el canal digital pasa de 36% a 69% del volumen y tiene 12,5% de mora contra 7,2% en sucursal). Además el criterio no es consistente: bajo esa misma lógica habría que eliminar `edad` y `score_buro`, que también derivan de forma marcada según los gráficos de §8.1. Se terminaría botando justamente la señal más valiosa.

**Qué hice.** Corregí la sección para dejar claro qué es la validación adversarial: un **diagnóstico de magnitud** de cuán distintos son los dos períodos, no un criterio de selección de variables. El único caso en que la deriva justifica excluir algo es cuando hay otra razón de fondo, como en `num_contactos_ult_trimestre` — que se excluye por ser una fuga, no por derivar. Rehíce el gráfico con solo dos escenarios (con y sin la fuga) y agregué la acción que sí corresponde: monitorear la mezcla de canal en producción para saber cuándo hay que reentrenar.

**El resultado interpretado bien.** El AUC adversarial baja de 0,65 a 0,60 al excluir la fuga: parte de la "diferencia" entre train y test era el artefacto de esa columna. El 0,60 restante es deriva real, y se maneja con el diseño de validación y la recalibración, no eliminando variables.

---

### 10. Verifiqué la monotonía con la herramienta equivocada

**Qué pasó.** Tras entrenar LightGBM con restricciones de monotonía en 7 variables, la IA propuso verificar que se cumplieran graficando el **efecto SHAP promedio por decil** de cada variable restringida. El chequeo dio **incumplimiento en tres variables** (`uso_linea_credito_pct`, `num_creditos_vigentes`, `ratio_deuda_ingreso`), lo que habría significado que las restricciones no estaban funcionando.

**Qué encontré al investigar.** El error era del método, no del modelo. SHAP reparte la predicción de cada caso entre las variables, absorbiendo interacciones con las demás; al agrupar por deciles se mezclan perfiles distintos, así que el promedio puede subir y bajar aunque cada predicción individual sea perfectamente monótona. Lo que la restricción garantiza es otra cosa: que al mover **solo** esa variable, dejando el resto fijo, la predicción se mueva siempre en la misma dirección.

**Qué hice.** Reemplacé la verificación por **dependencia parcial**: se recorre una grilla de valores de la variable, se aplica sobre una muestra real de solicitudes sin tocar nada más, y se promedia la predicción. Bajo este método —que es el que mide exactamente lo que la restricción promete— las **siete restricciones se cumplen sin excepción**. Dejé un `assert` en el notebook para que falle ruidosamente si eso deja de ser cierto tras un cambio futuro.

**Por qué importa.** De haber aceptado el primer resultado habría concluido que LightGBM no respetaba las restricciones y probablemente habría abandonado una decisión de diseño que resultó ser buena (la monotonía no cuesta AUC en estos datos: 0,8438 con restricciones contra 0,8423 sin ellas). SHAP explica *predicciones*; la dependencia parcial describe el *comportamiento del modelo*. Son preguntas distintas y usar la herramienta equivocada produce conclusiones equivocadas.
...
Queda por documentar la redacción del informe ejecutivo.
